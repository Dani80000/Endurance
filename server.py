...
CONNECTED_CLIENTS = set() # Keeps track of connected clients so messages are broadcasted to them
...
async def handler(websocket): # Async will allow us to wait for messages without blocking
    ...
    try: # This is here in order to void errors from getting bad data to prevent any failures
